	...
	for _, report := range reports {
		if areLevelsSafe(report.levels) {
			...
			for i := range report.levels {
				levelsWithCurrentLevelRemoved := getCopyOfSliceWithIndexRemoved(report.levels, i)
				if areLevelsSafe(levelsWithCurrentLevelRemoved) {
					sumOfSafeLevels++
					break
				}
			}
...
func getCopyOfSliceWithIndexRemoved(slice []int, indexToRemove int) []int {
	copyOfOriginalSlice := make([]int, len(slice))
	copy(copyOfOriginalSlice, slice)
	copyOfOriginalSlice = append(copyOfOriginalSlice[:indexToRemove], copyOfOriginalSlice[indexToRemove+1:]...)
	return copyOfOriginalSlice
}
